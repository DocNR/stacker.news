...
// came in via Caddy (https) or directly to localhost:3000 (http) — used to
// gate the Secure cookie attribute so localhost dev still works while the
// custom-domain HTTPS path exercises the full secure-cookie codepath.
//
...
export function isSecureRequest (req) {
  ...
  // TLS-terminating proxy (Caddy in dev, ALB in prod) is in front. On edge
  // NextRequest, req.nextUrl.protocol reflects the inbound hop to the app
  // (plain http behind Caddy), so we must consult the header first.
...
// constant work whether the request came in via HTTP localhost or HTTPS Caddy.
